...
**ASM Hydrasynth Explorer**, with the modern Fractal family (**Axe-Fx III /
FM3 / FM9**) and the original **Axe-Fx Standard/Ultra** in community
...
**40 MCP tools registered.** Unified surface for tone-building across all supported devices, plus generic-MIDI primitives and device-specific extensions.
...
- **Windows 10/11** (one-click installer). **macOS** via a one-command
  source build, see [docs/INSTALL-MAC.md](docs/INSTALL-MAC.md) (`npm run setup-mac`).
...
> **macOS:** `npm run setup-mac` builds and registers the server with
> Claude Desktop for you, no config editing. See
> [docs/INSTALL-MAC.md](docs/INSTALL-MAC.md). The Windows bootstrap above
> relies on PowerShell; Linux contributors still hand-edit
...
| `export_preset(port, directory?)` | Back up the active preset to a byte-exact `.syx` file on disk (Fractal AM4 + Axe-Fx II). Reads the working buffer as raw SysEx; reads only, never writes to the hardware. Point `directory` at a cloud-synced folder to reach other devices. Re-apply a backup with `apply_preset(port, restore_from)`. Other devices return capability_not_supported. |

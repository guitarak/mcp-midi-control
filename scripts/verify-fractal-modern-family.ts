 ...
 *
 * Run:  npx tsx scripts/verify-fractal-modern-family.ts
 */
...
async function readerChecks(): Promise<void> {
  ...
  // NO tail. The reader's read-until-quiet collector concatenates verbatim.
  {
    ...
    check('FM9 export dump format = fractal-modern-edit-buffer-dump', dump?.format === 'fractal-modern-edit-buffer-dump', dump?.format);
  }
  ...
  // coverage (the active-buffer dump above uses a different fn=0x43/0x51 path).
  {
    ...
    check('FM9 stored dump format = fractal-modern-stored-preset-dump', dump?.format === 'fractal-modern-stored-preset-dump', dump?.format);

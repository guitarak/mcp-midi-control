...
- **VP4** (model byte `0x14`): gen-3 codec but AM4-shape (serial 4-slot chain, 4 scenes, A-D channels, A01-Z04 locations, no amp/cab). Device-true catalog mined from VP4-Edit. Reads ship; all device-state writes are gated pending a serial block-placement capture. Community beta.

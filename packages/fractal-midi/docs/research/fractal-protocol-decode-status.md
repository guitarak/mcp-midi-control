...
| FM3 / FM9 / VP4 | `0x11` / `0x12` / `0x14` | Axe-Fx III family | (TBD if we add) | (TBD) | Not yet pursued. The III's overlay lift in `fractal-midi` (round-trip codec goldens + post-gen calibration overlay + enum vocabulary module + acceptance gate) is the template for FM3/FM9. See `fractal-midi-extraction-plan.md` §"Post-extraction, III calibration + FM3/FM9 add" for the per-step plan. Founder action needed: pull FM3-Edit + FM9-Edit installers and extract JUCE BinaryData into `samples/captured/decoded/binarydata/{fm3,fm9}-edit-allzips/extracted/`. |
...
1. **Verify the III responds to the byte-verified `fn=0x01` SET_PARAMETER envelope on real hardware** ( pivot): wire shape now byte-verified against 10 public captures (`docs/devices/axe-fx-iii/set-parameter-captures.md`); what remains is confirming the device actually honors AxeEdit III's captured wire when re-sent by our MCP tool, AND decoding the response shape (sync echo? STATE_BROADCAST? silent?). The project maintainer doesn't own a III. A III-owning contributor running `axefx3_set_parameter(block="Reverb 1", param_id=0, value=N)` against a scratch preset and reporting the audible effect + any inbound bytes closes the remaining 🟡 GET shape and confirms SET 🟢 end-to-end. See [`docs/AXEFX3-BETA-TESTING.md`](AXEFX3-BETA-TESTING.md) for the contributor on-ramp.
...
| 2 | fn 0x16 enum-count catalog reconciliation (II): the device's own fn 0x16 reports amp `G2` count = 265, but the on-disk fn 0x28 dump truncates at ~155 of 259 labels. **Tooling-blocked, not open-decode**: node-midi caps inbound SysEx at 2048 bytes. | Raise the receive buffer above 2048 (or concatenate inbound frames before NULL-split) and re-read fn 0x28 | No | Confirms whether the shipped 259-entry amp catalog is short vs an allocated-slot count of 265 |

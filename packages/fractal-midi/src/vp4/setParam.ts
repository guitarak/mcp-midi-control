...
function septets5LE(u: number): [number, number, number, number, number] {
  return [
    u & 0x7f,
    (u >>> 7) & 0x7f,
    (u >>> 14) & 0x7f,
...
/**
 * Encode a float32 as the VP4's on-wire 5-byte value: standard 5-septet LE with
 * the TOP TWO septets swapped (emit `[s0,s1,s2,s4,s3]`).
 */
 ...
 * `[s0,s1,s2,s4,s3]` (top two swapped); un-swap, reassemble, reinterpret.
 */

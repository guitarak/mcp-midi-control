...
server.registerTool('apply_patch', {
  ...
  description: [
    ...
    'FX SUB-PARAMS ARE TYPE-DEPENDENT. `prefxparam1..5` / `postfxparam1..5` mean different things per `prefxtype` / `postfxtype`. If you set an FX TYPE but omit its params, the encoder auto-fills that type\'s sane defaults (mined from factory patches) so the patch stays audible. Pass param1..5 to shape it. Per-type mappings:',
    ...
    '- ENV / LFO TIME params take DISPLAY time, like the front panel: a number in MILLISECONDS (e.g. 250) or a string ("2.5s" / "250ms" / "2.56 Sec"). Covers env*{attack,decay,release,hold,delay}syncoff + lfo*delaysyncoff. The codec inverts the device\'s non-linear time table internally (rounds to the nearest step). SLOW PAD: attack ~1500-3000ms, decay ~600ms, release ~2000-8000ms. SNAPPY BASS: attack 0-5ms, decay 30-200ms, release 30-150ms. env*sustain is a 0..128 LEVEL, not a time.',
    '- LFO FREE-RUN RATE (lfo*ratesyncoff) takes Hz like the panel: a number (4.44) or a string ("4.44 Hz"), 0.02..150 Hz. REVERB TIME (reverbtime) takes seconds: a number (2.6) or string ("2.6s"), "250ms" for sub-second tails, or "Freeze" for the infinite tail. Both invert the device\'s own table internally (nearest entry); pass the panel reading, never a raw index.',
    '- SYNC-ON variants are musical-division enum STRINGS: delaytimesyncon / lfo*ratesyncon / env*{...}syncon ("1/4", "1/4 D", "1/8", "1/16", …). Pair the sync flag (delaybpmsync:1 / lfo*bpmsync:1) with the matching syncon division. voicevibratoratesyncoff is still a raw 0..127 index (display-first pending).',

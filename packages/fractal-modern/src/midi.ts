...
function isCleanOutbound(bytes: readonly number[], modelId: number): boolean {
  if (isGen3Envelope(bytes, modelId)
    && bytes[5] === STORE_PRESET_FN && bytes[6] === STORE_PRESET_SUB) return true;
    ...
    try {
      ...
      for (let i = 0; i < out.getPortCount(); i++) {
        ...
        result.push({
          index: i,
          name,
          matched: spec.portNeedles.some((n) => lower.includes(n)),
        });
      }
      return result;
    } finally {

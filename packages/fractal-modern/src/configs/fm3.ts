 ...
 * now ships a DEVICE-TRUE param catalog (`fractal-midi/fm3`) mined from
...
export const FM3_CONFIG: FractalModernConfig = {
  ...
  verification:
    ...
    'filtered to FM3\'s families. The fn=0x01 SET round-trip is not yet FM3-hardware-confirmed (SET stays beta).',
  ...
  agent_guidance: {
    ...
    device_note: [
      ...
      'families. The fn=0x01 SET envelope is not yet FM3-hardware-confirmed,',
      'so confirm every write on the device.',
    ].join('\n'),
  },

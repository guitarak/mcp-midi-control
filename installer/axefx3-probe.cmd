...
rem MCP MIDI Control -Axe-Fx III read-back probe (READ-ONLY).
rem
rem Connects to a plugged-in Axe-Fx III over USB MIDI, runs a few read-only
...
rem reads. Quit Axe-Edit III first so it isn't holding the MIDI port.
rem
...
set "OUT=%USERPROFILE%\Desktop\axefx3-probe-output.json"

echo.
echo Running the Axe-Fx III read-back probe (READ-ONLY)...
echo Make sure your Axe-Fx III is connected and Axe-Edit III is closed.
echo.
